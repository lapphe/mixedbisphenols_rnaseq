...
#Dam nucleus accumbens deseq2 analysis and figures
...
group <- "dam"
cts <- read.csv("./data/dam_nac_raw.csv", row.names=1, check.names=FALSE)
coldata <- read.csv("./data/dam_condition_lg_data.csv", row.names=1)

#sample 20 has low counts (outlier), removing from analysis
cts <- subset(cts, select=-6)
coldata <- coldata %>% filter(dam_id != 20)
row.names(coldata) <- coldata$dam_id
