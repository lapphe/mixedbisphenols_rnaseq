...
#Pup amygdala deseq2 analysis and figures
...
brain_region_short <- "amy"
group <- "pup"
cts <- read.csv("./data/pup_amy_raw.csv", row.names=1, check.names=FALSE)

coldata <- read.csv("./data/pup_condition_lg_data.csv", row.names=1)

#sample 19.6 is an outlier, so removing it from analysis
cts <- cts[, -10] #19.6 is the 10th column
names.remove <- c("19.6")
coldata <- coldata[!(row.names(coldata) %in% names.remove),]
...
# -----------------Create pheatmap with top 100 most variable genes- (fig 3)
...
exposures <- coldata %>%  dplyr::select(condition, sex, lg_centered)
colnames(exposures) <- c("Condition", "Sex", "Licking/grooming")
...
ann_colors = list(
  ...
  Sex = c(female = "honeydew3", male = "black"),
...
h <- pheatmap(data_subset_norm, 
              ...
              fontsize = 9,
              col = colorRampPalette(c("seagreen", "white", "darkgoldenrod1"))(50)) 
h
ggsave(h, filename = paste(brain_region, "_pheatmap _pup.png", sep = ""), path = "./figures_tables/", dpi= 600, height = 5, width = 2 )
...
pca_plot
ggsave(pca_plot, filename = paste(brain_region, "_pca_pup.png", sep = ""), path = "./figures_tables/", dpi= 600, height = 2, width = 2 )
...
d
ggsave(d, filename = paste(brain_region, "_QQ_pup.png", sep = ""), path = "./figures_tables/", dpi= 600, height = 2, width = 2 )
