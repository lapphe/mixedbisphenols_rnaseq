...
#BP pilot dams RNAseq DEGs for 5 regions figure for unadjusted p val <.05
...
amy1 <- read.csv("./results/dam-amy-all-results-BP.csv") %>% 
  filter(pvalue < .05)
hipp1 <- read.csv("./results/dam-hipp-all-results-BP.csv") %>% 
  filter(pvalue < .05)
pl1 <- read.csv("./results/dam-pl-all-results-BP.csv") %>% 
  filter(pvalue < .05)
nac1 <- read.csv("./results/dam-nac-all-results-BP.csv") %>% 
  filter(pvalue < .05)
hypo1 <- read.csv("./results/dam-hypo-all-results-BP.csv") %>% 
  filter(pvalue < .05)
...
df%>%
  ...
  scale_fill_gradient2(low="navy", 
                       mid="white", 
                       high="darkorange2", #colors in the scale
  ...
  ggtitle("Differentially expressed genes for bisphenol exposure in dams")+
  ...
  theme(legend.position = c(-0.18,-0.04),
        ...
        strip.text.y.left = element_text(angle = 0, size = 12)) -> plot1
plot1

ggsave(plot1, filename = "dam_bp_unionpheatmap.png", path = "./figures_tables/", dpi= 600, height = 2, width = 6 )


#----------------LG figure 5 region figure--
#filter all licking/grooming deseq2 results by pvalue <.05
amy2 <- read.csv("./results/dam-amy-all-results-LG.csv") %>% 
  filter(pvalue < .05)
hipp2 <- read.csv("./results/dam-hipp-all-results-LG.csv") %>% 
  filter(pvalue < .05)
pl2<- read.csv("./results/dam-pl-all-results-LG.csv") %>% 
  filter(pvalue < .05)
nac2<- read.csv("./results/dam-nac-all-results-LG.csv") %>% 
  filter(pvalue < .05)
hypo2 <- read.csv("./results/dam-hypo-all-results-LG.csv") %>% 
  filter(pvalue < .05)
...
df%>%
  ...
  scale_y_discrete(labels = y_axis)+
  ggtitle("Differentially expressed genes for licking/grooming in dams")+
  theme_bw(base_size = 9)+
  theme(legend.position = c(-0.18,-0.04),
        ...
        panel.background = element_rect(fill = "transparent",colour = "white"),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        strip.text.y.left = element_text(angle = 0, size = 12)) -> plot2
plot2

ggsave(plot2, filename = "dam_lg_unionpheatmap.png", path = "./figures_tables/", dpi= 600, height = 2, width = 6 )
