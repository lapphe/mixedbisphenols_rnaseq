...
#Dam hypothalamus deseq2 analysis and figures
...
group <- "dam"
cts <- read.csv("./data/dam_hypo_raw.csv", row.names=1, check.names=FALSE)
coldata <- read.csv("./data/dam_condition_lg_data.csv", row.names=1)


coldata <- coldata %>% filter(coldata$dam_id != "23") #sample 23 is not included in this brain region, update coldata
